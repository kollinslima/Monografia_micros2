...
\par O método utilizada para realizar este trabalho baseia-se no que foi utilizado em~\cite{Barros},~\cite{Yeo} e~\cite{Chen}. Basicamente o processo se divide conforme mostra a figura~\ref{fig:processo}.
...
\par Experimentalmente, chegou-se ao valor de 256-bits para o tamanho do \textit{hash}, abaixo deste valor não era possível diferenciar as imagens com o classificador utilizado. A figura~\ref{fig:exemplo_hash} mostra um exemplo de como o \textit{hash} atua na imagem binarizada mostarda na figura~\ref{fig:imagem_pre_proc}.
...
\par Por meio da interface \textit{web} desenvolvida, mostrada na figura~\ref{fig:website}, é possível visualizar cada imagem recebida pela Raspberry e fazer o treinamento do banco de dados relacionando a imagem visualizada ao seu número correspondente.

\begin{figure}
	\centering
	\includegraphics[width=0.7\textwidth]{./Resources/pagina_treinamento.png}
	\caption{Página \textit{web} para treinamento do banco de dados.}
	\label{fig:website}
\end{figure}
